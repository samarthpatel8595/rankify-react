...
const Sidebar = ({ isOpen = false, onClose }: SidebarProps) => {
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.label}
            href={item.href}
            className={`group flex items-center gap-3 rounded-xl px-4 py-3 text-base font-medium transition ${isActive
                ? "bg-white/10 text-sunray"
                : "text-white/70 hover:bg-white/5 hover:text-white"
              }`}
            onClick={onClose}
          >
            {Icon === "podcasts" || Icon === "events" || Icon === "group" ? (
              <Image
                src={
                  Icon === "podcasts"
                    ? isActive
                    ...
                    : Icon === "events"
                      ? isActive
                        ? "/icons/Capa_1-gold.svg"
                        : "/icons/Capa_1-white.svg"
                      : Icon === "group"
                      ? isActive
                        ? "/icons/Group-gold.svg"
                        : "/icons/Group-white.svg"
                        : ""
                }
                alt={item.label}
                width={24}
  ...
  return (
    <>
      <aside className="hidden md:flex h-screen w-full flex-col gap-8 bg-ink px-6 pb-10 pt-8 text-floral sticky top-0">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="h-9 w-9 flex items-center justify-center rounded-lg border border-white/10 text-white"
          >
            <X size={18} />
          </button>
        </div>
